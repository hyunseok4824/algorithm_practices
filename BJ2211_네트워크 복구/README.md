...
1. 해커가 다시 공격을 할 우려가 있기 때문에, 최소 개수의 회선만을 복구해야 한다. 물론, 그렇다면 아무 회선도 복구하지 않으면 되겠지만, 이럴 경우 네트워크의 사용에 지장이 생기게 된다. 
따라서 네트워크를 복구한 후에 서로 다른 두 컴퓨터 간에 통신이 가능하도록 복구해야 한다.
2. 네트워크를 복구해서 통신이 가능하도록 만드는 것도 중요하지만, 해커에게 공격을 받았을 때 보안 패킷을 전송하는 데 걸리는 시간도 중요한 문제가 된다. 
...
## 02. 문제 풀이
이 문제는 기본적인 다익스트라 알고리즘을 사용해서 최단 거리를 구하는 문제였다. 
다만 단순히 최소 비용만 구하는 것이 아니라 목적지까지의 경로를 어떻게 추적할 것인지도 중요했다.
이를 위해, 각 노드가 갱신될 때 이전 노드를 저장하는 parents 리스트를 추가로 두었다. 
다익스트라 알고리즘에서 어떤 노드의 최단 거리가 갱신되면, 해당 노드에 도달하기 직전의 노드를 parents에 기록한다. 
이렇게 하면 최단 거리 계산이 끝난 후, 모든 노드를 연결하는 **최소 개수의 간선**을 파악할 수 있게 된다.
이는 다익스트라가 DP(Dynamic Programming)를 활용한 알고리즘이며, 이미 계산된 값이 최적의 값이라는 전제에서 진행되었기 때문에 가능하다.

다익스트라 알고리즘이 끝난 후, for 문을 통해서 노드 i와 i의 부모 노드를 출력하면 된다.